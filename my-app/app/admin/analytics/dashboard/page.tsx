...
interface AnalyticsData {
  ...
  occupancy: {
    daily: Array<{ date: string; rate: number; available: number; booked: number; }>;
    average: number;
    trend: number;
  };
  pricing: {
    trends: Array<{ date: string; avgPrice: number; minPrice: number; maxPrice: number; }>;
...
export default function AdminAnalyticsDashboard() {
  ...
  const [properties, setProperties] = useState<Array<{id: string; name: string; location: string}>>([]);
  const [events, setEvents] = useState<Array<{id: string; name: string; date: string}>>([]);
  const [rules, setRules] = useState<Array<{id: string; name: string; type: string}>>([]);
      ...
      <div className="flex items-center justify-between">
        <div className="space-y-1">
          ...
          <p className="text-muted-foreground">
            Monitor revenue, occupancy, and pricing performance across all properties
          </p>
        </div>
            ...
            <div className="space-y-2">
              ...
              <DatePickerWithRange
                ...
                onDateChange={(range) =>
                  setFilters(prev => ({
                    ...prev,
                    dateRange: range && typeof range === 'object' ? { from: range.from ?? subDays(new Date(), 30), to: range.to ?? new Date() } : { from: subDays(new Date(), 30), to: new Date() }
                  }))
                }
              />
            </div>
            ...
            <CardContent className="p-6">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-bold">₹{(analyticsData.revenue.total / 100000).toFixed(1)}L</p>
                </div>
            ...
            <CardContent className="p-6">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-bold">{analyticsData.occupancy.average}%</p>
                </div>
          ...
          <Card>
            <CardContent className="p-6">
                ...
                <div>
                  ...
                  <p className="text-2xl font-bold">
                    ₹{analyticsData.pricing.trends.length > 0 
                      ? Math.round(analyticsData.pricing.trends.reduce((sum, day) => sum + day.avgPrice, 0) / analyticsData.pricing.trends.length).toLocaleString()
                      : '0'
                    }
                  </p>
                </div>
              ...
              <div className="flex items-center mt-2">
                ...
                <span className="text-sm text-purple-600 font-medium">+12.3%</span>
                <span className="text-sm text-muted-foreground ml-2">vs last month</span>
              </div>
            </CardContent>
          </Card>
        ...
        <TabsContent value="overview" className="space-y-6">
              ...
              <CardContent>
                <ResponsiveContainer width="100%" height={300}>
                  <AreaChart data={analyticsData?.revenue.daily || []}>
                    ...
                    <Tooltip 
                      labelFormatter={(value: string) => format(parseISO(value), 'MMM dd, yyyy')}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                  ...
                  {analyticsData?.topMovers.priceChanges.map((property) => (
                    <div key={property.id} className="flex items-center justify-between p-3 border rounded-lg">
                      <div className="flex-1">
                        ...
                        <p className="text-xs text-muted-foreground mt-1">{property.impact}</p>
                      </div>
            ...
            <Card>
              ...
              <CardContent>
                <div className="space-y-4">
                  {analyticsData?.topMovers.performance.map((property, index) => (
                    <div key={property.id} className="flex items-center justify-between p-3 border rounded-lg">
            ...
            <CardContent>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                {analyticsData?.events.map((event) => (
                  <div key={event.id} className="p-4 border rounded-lg bg-gradient-to-br from-indigo-50 to-purple-50">
                    <h4 className="font-medium text-indigo-900">{event.name}</h4>
                    ...
                    <div className="space-y-2 text-sm">
                      <div className="flex justify-between">
                        ...
                        <span className="font-medium text-indigo-800">+{event.impact}%</span>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-indigo-600">Bookings:</span>
                        <span className="font-medium text-indigo-800">{event.bookings}</span>
                      </div>
                      <div className="flex justify-between">
                ...
                <ResponsiveContainer width="100%" height={400}>
                  <ComposedChart data={analyticsData?.revenue.daily || []}>
                    ...
                    <XAxis 
                      ...
                      tickFormatter={(value: string) => format(parseISO(value), 'MMM dd')}
                    />
                    <YAxis yAxisId="revenue" orientation="left" tickFormatter={(value: number) => `₹${(value/1000).toFixed(0)}K`} />
                    <YAxis yAxisId="bookings" orientation="right" />
                    <Tooltip 
                      labelFormatter={(value: string) => format(parseISO(value), 'MMM dd, yyyy')}
                    />
                    <Legend />
                    <Bar yAxisId="revenue" dataKey="revenue" fill="#22c55e" name="Revenue" />
                    <Line yAxisId="bookings" type="monotone" dataKey="bookings" stroke="#3b82f6" strokeWidth={2} name="Bookings" />
                  </ComposedChart>
                </ResponsiveContainer>
              </CardContent>
            </Card>
              ...
              <ResponsiveContainer width="100%" height={400}>
                <ComposedChart data={analyticsData?.occupancy.daily || []}>
                  ...
                  <XAxis 
                    ...
                    tickFormatter={(value: string) => format(parseISO(value), 'MMM dd')}
                  />
                  <YAxis yAxisId="rate" domain={[0, 100]} tickFormatter={(value: number) => `${value}%`} />
                  <YAxis yAxisId="rooms" orientation="right" />
                  <Tooltip 
                    labelFormatter={(value: string) => format(parseISO(value), 'MMM dd, yyyy')}
                  />
                  <Legend />
                  <Area yAxisId="rate" type="monotone" dataKey="rate" stroke="#3b82f6" fill="#3b82f6" fillOpacity={0.3} name="Occupancy Rate" />
                  <Bar yAxisId="rooms" dataKey="booked" fill="#22c55e" name="Booked Rooms" />
                  <Bar yAxisId="rooms" dataKey="available" fill="#e5e7eb" name="Available Rooms" />
                </ComposedChart>
              </ResponsiveContainer>
            </CardContent>
          </Card>
              ...
              <CardContent>
                <ResponsiveContainer width="100%" height={400}>
                  <LineChart data={analyticsData?.pricing.trends || []}>
                    ...
                    />
                    <Legend />
                    <Line type="monotone" dataKey="maxPrice" stroke="#ef4444" strokeWidth={1} name="Max Price" strokeDasharray="5 5" />
                    <Line type="monotone" dataKey="avgPrice" stroke="#3b82f6" strokeWidth={3} name="Avg Price" />
                    <Line type="monotone" dataKey="minPrice" stroke="#22c55e" strokeWidth={1} name="Min Price" strokeDasharray="5 5" />
                  </LineChart>
                </ResponsiveContainer>
              </CardContent>
            </Card>
                ...
                <ResponsiveContainer width="100%" height={400}>
                  <PieChart>
                    <Pie
                      ...
                      dataKey="count"
                    >
                      {analyticsData?.pricing.distribution.map((entry, index) => (
