...
export async function PUT(
  ...
  try {
    ...
    console.log('[Plan Pricing API] Received entries:', pricingEntries.length)
    ...
    for (const entry of pricingEntries) {
      try {
        ...
        console.log(`[Plan Pricing API] Processing ${planType}-${occupancyType}: price=${price}, isAvailable=${isAvailable}`)

        // FIXED: Check for undefined/null instead of falsy (to allow price=0)
        if (!roomCategory || !planType || !occupancyType || price === undefined || price === null) {
          errors.push({ entry, error: 'Missing required fields' })
          console.log(`[Plan Pricing API] SKIPPED ${planType}-${occupancyType}: Missing required fields`)
          continue
        }
        ...
        console.log(`[Plan Pricing API] SAVED ${planType}-${occupancyType}: price=${result.price}, isAvailable=${result.isAvailable}`)
