...
async function getRevenueAnalytics(userId: string) {
  try {
    ...
    console.log('[Reports] User ID:', userId, 'Property IDs:', propertyIds);
    ...
    const revenueAgg = await Booking.aggregate([
      { $match: query },
      {
        $group: {
          ...
          hotelRevenue: {
            $sum: {
              $cond: [{ $eq: ['$hotelPaymentStatus', 'collected'] }, '$hotelPaymentAmount', 0]
            }
          },
          pendingRevenue: {
            $sum: {
              $cond: [{ $eq: ['$hotelPaymentStatus', 'pending'] }, '$hotelPaymentAmount', 0]
            }
          },
          totalBookings: { $sum: 1 }
        }
    ...
    const growth = previousRevenue > 0 ? ((summary.totalRevenue - previousRevenue) / previousRevenue) * 100 : 0;
    ...
    return JSON.parse(JSON.stringify({
      ...
      summary: {
        ...
        averageBookingValue: summary.totalBookings > 0 ? Math.round(summary.totalRevenue / summary.totalBookings) : 0
      },
...
export default async function OwnerReportsPage() {
  ...
  const data = await getRevenueAnalytics(session.user!.id!);
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <Link href="/os/reports/analytics" className="px-4 py-2 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 transition-colors flex items-center gap-2">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
          ...
          <div className="flex items-center gap-1 mt-2">
            {summary.growth >= 0 ? (
              <>
                ...
                <p className="text-sm text-green-600">+{summary.growth}% from previous period</p>
              </>
            ) : (
              <>
                <TrendingDown className="w-4 h-4 text-red-600" />
                <p className="text-sm text-red-600">{summary.growth}% from previous period</p>
              </>
            )}
          </div>
        </div>
        ...
        <div className="bg-white rounded-lg shadow-sm p-6 border border-gray-200">
          ...
          <p className="text-3xl font-bold text-gray-900">{formatCurrency(summary.onlineRevenue)}</p>
          <p className="text-xs text-gray-500 mt-2">
            {summary.totalRevenue > 0 ? Math.round((summary.onlineRevenue / summary.totalRevenue) * 100) : 0}% of total
          </p>
        </div>
        ...
        <div className="bg-white rounded-lg shadow-sm p-6 border border-gray-200">
          ...
          <p className="text-3xl font-bold text-gray-900">{formatCurrency(summary.hotelRevenue)}</p>
          <p className="text-xs text-gray-500 mt-2">Collected at property</p>
        </div>
        ...
        <div className="bg-white rounded-lg shadow-sm p-6 border border-gray-200">
          ...
          <p className="text-3xl font-bold text-gray-900">{formatCurrency(summary.pendingRevenue)}</p>
          <p className="text-xs text-gray-500 mt-2">Yet to be collected</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="bg-white rounded-lg shadow-sm p-6 border border-gray-200">
          ...
          <p className="text-3xl font-bold text-gray-900">{formatCurrency(summary.averageBookingValue)}</p>
        ...
        <div className="bg-white rounded-lg shadow-sm p-6 border border-gray-200">
          ...
          <p className="text-3xl font-bold text-gray-900">
            {(summary.hotelRevenue + summary.pendingRevenue) > 0
              ? Math.round((summary.hotelRevenue / (summary.hotelRevenue + summary.pendingRevenue)) * 100)
              : 100}%
          </p>
                  ...
                  <tr key={property.propertyId} className="hover:bg-gray-50">
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="font-medium text-gray-900">{property.propertyTitle}</span>
                      </div>
                    </td>
                    <td className="px-6 py-4 text-gray-900">{property.bookings}</td>
                    <td className="px-6 py-4 font-semibold text-gray-900">{formatCurrency(property.amount)}</td>
                    <td className="px-6 py-4 text-gray-900">{formatCurrency(property.amount / property.bookings)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
                ...
                return (
                  <div key={day.date}>
                    <div className="flex items-center justify-between mb-1">
                      ...
                      <span className="text-sm font-semibold text-gray-900">{formatCurrency(day.amount)}</span>
                    </div>
      ...
      <div className="bg-gradient-to-br from-indigo-50 to-purple-50 rounded-lg p-6 border border-indigo-100">
        <div className="flex items-start justify-between">
          <div>
            ...
            <p className="text-sm text-gray-700 mb-3">Explore your data with interactive charts and customizable date ranges</p>
          ...
          <Link href="/os/reports/analytics" className="px-6 py-3 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 transition-colors font-medium flex items-center gap-2 whitespace-nowrap">
            <BarChart3 className="w-5 h-5" />
            View Analytics
          </Link>
        </div>
