...
export default function AiPricingSuggestions({
  ...
  const [suggestionsData, setSuggestionsData] =
    useState<AiSuggestionsData | null>(null)
  ...
  const fetchSuggestions = useCallback(async () => {
    ...
    try {
      ...
      const response = await fetch(
        `/api/admin/properties/${propertyId}/ai-pricing-suggestions?${params}`
      )
  ...
  }, [propertyId, startDate, endDate, guests, fetchSuggestions])
  ...
  const { suggestions, marketAnalysis, historicalInsights, recommendations } =
    suggestionsData
        ...
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div className="text-center p-3 rounded-lg">
              <Badge
                className={getDemandLevelColor(marketAnalysis.demandLevel)}
              >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {suggestions.map((suggestion, index) => {
          ...
          return (
            <Card
              key={suggestion.type}
              className={`cursor-pointer transition-all hover:shadow-md ${getSuggestionColor(
                suggestion.type
              )}`}
            >
              <CardHeader
                onClick={() =>
                  setExpandedCard(isExpanded ? null : suggestion.type)
                }
                className="pb-3"
              >
                ...
                <div className="space-y-3">
                  ...
                  <div className="flex items-center justify-between text-sm">
                    ...
                    <span className="font-medium">
                      {Math.round(suggestion.expectedOccupancy * 100)}%
                    </span>
                  </div>
                  ...
                  <Button
                    ...
                    variant={
                      suggestion.type === "optimal" ? "default" : "outline"
                    }
                  >
                    Use This Price
                  </Button>
            ...
            <div>
              ...
              <div className="space-y-2">
                <div className="flex justify-between text-sm">
                  <span>Minimum:</span>
                  <span className="font-medium">
                    ₹{marketAnalysis.priceRange.min}
                  </span>
                </div>
                <div className="flex justify-between text-sm">
                  <span>Maximum:</span>
                  <span className="font-medium">
                    ₹{marketAnalysis.priceRange.max}
                  </span>
                </div>
                <div className="flex justify-between text-sm">
              ...
              <div>
                ...
                <div className="space-y-2">
                  ...
                  <div className="flex justify-between text-sm">
                    ...
                    <span className="font-medium">
                      {Math.round(historicalInsights.occupancyRate * 100)}%
                    </span>
                  </div>
                  <div className="flex justify-between text-sm">
                    <span>Seasonal Data:</span>
                    <span className="font-medium">
                      {historicalInsights.seasonalTrends.length} months
                    </span>
                  </div>
                </div>
              </div>
