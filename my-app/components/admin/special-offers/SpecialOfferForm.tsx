...
export function SpecialOfferForm({
  ...
  const form = useForm<SpecialOfferFormValues>({
    ...
    defaultValues: {
      ...
      targetProperties: Array.isArray(offer?.targetProperties)
        ? offer.targetProperties.join(", ")
        : "",
    },
  })
  ...
  const onSubmit = (data: SpecialOfferFormValues) => {
    const parsedTargetProperties = data.targetProperties
      ?.split(",")
      .map((id) => id.trim())
      .filter(Boolean) as unknown as ISpecialOffer["targetProperties"]

    onSave({
      ...data,
      targetProperties: parsedTargetProperties,
    })
  }
    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-4xl grid-cols-1 md:grid-cols-2">
        <div className="p-4 space-y-4">
          <DialogHeader>
            <DialogTitle>
              {offer?._id ? "Edit Special Offer" : "Create Special Offer"}
            </DialogTitle>
            <DialogDescription>
          ...
          >
            ...
            <div>
              <Label htmlFor="targetProperties">
                Target Property IDs (comma-separated)
              </Label>
              <Input
                id="targetProperties"
                {...form.register("targetProperties")}
            ...
            <div className="flex items-center justify-between">
              <div className="flex flex-col space-y-2">
                ...
                <Popover>
                  <PopoverTrigger asChild>
                    <Button
                      ...
                      className={cn(
                        ...
                        !watchAllFields.validUntil && "text-muted-foreground"
                      )}
                    >
                      <CalendarIcon className="mr-2 h-4 w-4" />
                      {watchAllFields.validUntil ? (
                        format(watchAllFields.validUntil, "PPP")
                      ) : (
                  ...
                  <PopoverContent className="w-auto p-0" align="start">
                    <Calendar
                      mode="single"
                      selected={watchAllFields.validUntil}
                      onSelect={(date) =>
              ...
              <div className="flex items-center space-x-2 pt-6">
                <Switch
                  id="isActive"
                  checked={watchAllFields.isActive}
                ...
                <div className="flex justify-center px-6 pt-5 pb-6 border-2 border-gray-300 border-dashed rounded-md">
                  <div className="space-y-1 text-center">
                    ...
                    <div className="flex text-sm text-gray-600">
                      <label
                        htmlFor="file-upload"
                        className="relative cursor-pointer bg-white rounded-md font-medium text-darkGreen hover:text-darkGreen/80 focus-within:outline-none"
                      >
        ...
        <div className="p-4 bg-gray-50 rounded-lg flex flex-col items-center justify-center">
          ...
          <SpecialOfferCard
            {...watchAllFields}
            validUntil={watchAllFields.validUntil || new Date()}
            isLivePreview
          />
        </div>
